...
export function Messages() {
  ...
  const [mobileView, setMobileView] = useState<'list' | 'chat'>('list'); // Mode hiển thị trên Mobile
  ...
  // Hide the floating Messenger chatbox when on the Messages page
  useEffect(() => {
    const btn = document.getElementById('floating-messenger-btn');
  ...
  useEffect(() => {
    ...
    const realtimeChannel = supabase.channel('realtime-msgs')
      ...
      .on('postgres_changes', { event: 'UPDATE', schema: 'public', table: 'messages', filter: `company_id=eq.${companyId}` }, (payload) => {
  ...
  const handleCatchCustomer = async (senderId: string) => {
    if (!user || user.role !== 'sale') return;
    const currentUserName = user.name || user.email;

    let unrepliedCount = 0;
    convMap.forEach(conv => {
      if (conv.assignee === currentUserName) {
        const hasReplied = conv.messages.some((m: any) => m.is_from_bot && m.staff_name === currentUserName);
        if (!hasReplied) unrepliedCount++;
      }
    });

    if (unrepliedCount >= 3) {
      alert('Bạn đang có 3 khách chưa phản hồi.\nHãy nhắn tin cho người cũ trước khi Bắt thêm khách mới!');
      return;
    }

    try {
      await supabase.from('messages').update({ assigned_staff_name: currentUserName }).eq('sender_id', senderId).eq('company_id', companyId);
      setMessages(prev => prev.map(m => m.sender_id === senderId ? { ...m, assigned_staff_name: currentUserName } : m));
      // Khách sẽ biến mất khỏi mảng filteredConversations vì thuộc tính assignee đã được set
    } catch (e) {
      console.error(e);
    }
  };
  ...
  const allConversations = Array.from(convMap.values()).map(conv => {
    ...
    // Ưu tiên lấy tên thật (không phải "Khách hàng") từ message mới nhất
    const realNameMsg = [...conv.messages].reverse().find((m: any) => m.sender_name && !m.sender_name.startsWith('Khách hàng') && !m.is_from_bot);
    const fallbackNameMsg = [...conv.messages].reverse().find((m: any) => m.sender_name && !m.is_from_bot);
    const dbName = realNameMsg?.sender_name || fallbackNameMsg?.sender_name;
    const dbAvatar = [...conv.messages].reverse().find((m: any) => m.sender_avatar)?.sender_avatar;
  ...
  const filteredConversations = allConversations.filter(c => {
    // Nếu là Sale, ẩn các hội thoại đã được bắt
    if (user?.role === 'sale' && c.assignee) return false;
  ...
  return (
    <div className="h-[calc(100vh-8rem)] flex flex-col space-y-4">
      ...
      {/* Top Filter Bar - hidden on mobile when viewing chat */}
      <div className={cn("flex md:items-center flex-col md:flex-row gap-3 p-3 bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-lg shadow-sm shrink-0 overflow-x-auto whitespace-nowrap scrollbar-hide", mobileView === 'chat' && 'hidden md:flex')}>
      ...
      {/* Stats Row - hidden on mobile when viewing chat */}
      <div className={cn("grid grid-cols-2 md:grid-cols-4 gap-2 md:gap-4 shrink-0", mobileView === 'chat' && 'hidden md:grid')}>
        <Card className="!p-2.5 md:!p-4 flex flex-col justify-between border-l-4 border-l-gray-300">
          <div className="flex justify-between items-center mb-0.5 md:mb-2">
            <span className="text-xs md:text-sm font-medium text-gray-500 dark:text-gray-400">Tổng tin nhắn</span>
          </div>
          <span className="text-base md:text-2xl font-bold text-gray-900 dark:text-white">{allConversations.length}</span>
        </Card>
        <Card className="!p-2.5 md:!p-4 flex flex-col justify-between border-l-4 border-l-red-400">
          <div className="flex justify-between items-center mb-0.5 md:mb-2">
            <span className="text-xs md:text-sm font-medium text-gray-500 dark:text-gray-400">Nóng</span>
            <Flame className="w-3.5 h-3.5 md:w-4 md:h-4 text-red-500" />
          </div>
          <span className="text-base md:text-2xl font-bold text-gray-900 dark:text-white">{statNong}</span>
        </Card>
        <Card className="!p-2.5 md:!p-4 flex flex-col justify-between border-l-4 border-l-amber-400">
          <div className="flex justify-between items-center mb-0.5 md:mb-2">
            <span className="text-xs md:text-sm font-medium text-gray-500 dark:text-gray-400">Tiềm năng</span>
            <Zap className="w-3.5 h-3.5 md:w-4 md:h-4 text-amber-500" />
          </div>
          <span className="text-base md:text-2xl font-bold text-gray-900 dark:text-white">{statTiemNang}</span>
        </Card>
        <Card className="!p-2.5 md:!p-4 flex flex-col justify-between border-l-4 border-l-gray-400">
          <div className="flex justify-between items-center mb-0.5 md:mb-2">
            <span className="text-xs md:text-sm font-medium text-gray-500 dark:text-gray-400">Spam</span>
            <Ban className="w-3.5 h-3.5 md:w-4 md:h-4 text-gray-400" />
          </div>
          <span className="text-base md:text-2xl font-bold text-gray-900 dark:text-white">{statSpam}</span>
        </Card>
      </div>

      <Card className={cn("flex-1 flex overflow-hidden p-0 relative", mobileView === 'chat' ? 'fixed inset-0 z-50 md:relative md:z-0 md:inset-auto h-full rounded-none md:rounded-xl' : 'h-full')}>
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex justify-between items-start mb-1">
                        <div className="flex items-center gap-2 max-w-[65%]">
                          ...
                          {user?.role !== 'sale' && conv.assignee && (
                            <span className="text-[9px] px-1.5 py-0.5 whitespace-nowrap bg-brand-blue/10 text-brand-blue rounded border border-brand-blue/20">
                              {conv.assignee}
                            </span>
                          )}
                        </div>
        ...
        <div className={cn(
          ...
          mobileView === 'list' ? 'hidden md:flex' : 'flex h-full'
        )}>

          {activeConv ? (
            <>
              {/* Main header inside right panel */}
              <div className="shrink-0 border-b border-gray-200 dark:border-gray-800 bg-white dark:bg-gray-900 shadow-sm md:shadow-none z-20 sticky top-0">
                {/* Row 1: User info */}
                <div className="h-12 md:h-16 px-2 md:px-6 flex items-center justify-between">
                  <div className="flex items-center gap-1.5 md:gap-3 min-w-0 flex-1">
                    <button 
                      className="md:hidden p-1 text-gray-500 hover:bg-gray-100 dark:hover:bg-gray-800 rounded-lg shrink-0"
                      onClick={() => setMobileView('list')}
                    >
                      <ChevronLeft className="w-5 h-5" />
                    </button>
                    <div className="w-8 h-8 md:w-10 md:h-10 rounded-full bg-blue-100 dark:bg-blue-900/50 text-brand-blue flex items-center justify-center font-semibold text-xs md:text-sm shrink-0 overflow-hidden">
                    ...
                    <div className="min-w-0 flex-1">
                      <h3 className="font-semibold text-sm md:text-base text-gray-900 dark:text-white truncate">{activeConv.name}</h3>
                      <div className="flex items-center gap-1.5 mt-0.5">
                        <span className="text-[10px] md:text-xs text-brand-blue flex items-center gap-0.5 shrink-0">
                          {activeConv.platform === 'facebook' ? <FacebookIcon width={10} height={10} /> : <div className="w-2.5 h-2.5 md:w-3 md:h-3 rounded bg-blue-500 text-white flex items-center justify-center text-[8px] md:text-[10px] leading-none font-bold">Z</div>}
                          <span className="hidden md:inline">{activeConv.channelName}</span>
                        </span>
                        ...
                        <select
                          ...
                          className={cn(
                            "text-[10px] md:text-[11px] px-1.5 md:px-2 py-0.5 rounded font-medium border cursor-pointer focus:outline-none",
                  ...
                  <div className="hidden md:flex items-center gap-3 shrink-0">
                    <div className="flex items-center gap-2 bg-gray-100 dark:bg-gray-800 rounded-lg px-3 py-1.5 border border-gray-100 dark:border-gray-700">
                      <span className="text-[10px] font-bold text-gray-500 uppercase tracking-wider">Trực Ca:</span>
                      <select
                        className="bg-transparent border-none text-xs font-bold focus:ring-0 text-brand-blue dark:text-blue-400 p-0 cursor-pointer min-w-[100px] outline-none"
                        value={currentShiftName}
                        onChange={(e) => handleAssignChange(e.target.value)}
                      >
                        <option value="">{user?.name || 'Chưa định danh'}</option>
                ...
                {/* Row 2: Tabs - compact on mobile */}
                {(user?.role === 'owner' || user?.role === 'admin') && (
                  <div className="px-2 md:px-6 pb-2 flex items-center">
                    <div className="flex bg-gray-100 dark:bg-gray-800 p-0.5 md:p-1 rounded-lg border border-gray-200 dark:border-gray-700">
                      <button
                        onClick={() => setActiveTab('chat')}
                        className={cn(
                          "flex items-center gap-1 md:gap-2 px-2.5 md:px-4 py-1 md:py-1.5 rounded-md text-xs md:text-sm font-medium transition-colors",
                          activeTab === 'chat' ? 'bg-white dark:bg-gray-700 text-brand-blue shadow-sm' : 'text-gray-500 hover:text-gray-900'
                        )}
                      >
                        <MessageSquare className="w-3.5 h-3.5 md:w-4 md:h-4" /> Tin nhắn
                      </button>
                      <button
                        onClick={() => setActiveTab('evaluate')}
                        className={cn(
                          "flex items-center gap-1 md:gap-2 px-2.5 md:px-4 py-1 md:py-1.5 rounded-md text-xs md:text-sm font-medium transition-colors",
                          activeTab === 'evaluate' ? 'bg-white dark:bg-gray-700 text-purple-600 shadow-sm' : 'text-gray-500 hover:text-gray-900'
                        )}
                      >
                        <ShieldCheck className="w-3.5 h-3.5 md:w-4 md:h-4" /> Đánh giá
                        {activeConv.ai_evaluation && <div className="w-1.5 h-1.5 md:w-2 md:h-2 rounded-full bg-purple-500" />}
                      </button>
                    </div>
                  </div>
                )}
              ...
              {activeTab === 'chat' ? (
                <>
                  ...
                  <div className="p-4 bg-white dark:bg-gray-900 border-t border-gray-200 dark:border-gray-800 shrink-0">
                    {user?.role === 'sale' ? (
                      <Button 
                        onClick={() => handleCatchCustomer(activeConv.sender_id)}
                        className="w-full font-bold h-[46px] rounded-xl text-[15px]"
                      >
                        👋 Bắt lấy khách hàng này
                      </Button>
                    ) : (
